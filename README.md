...
- 資産（スキル28件・GAS7件以上・スプレッドシート多数）が**ひとつもバージョン管理下にない**
- マイドライブ直下に**250件超**が平置きされ、うち「無題のスプレッドシート」だけで15件以上
- スキル4件が参照先を**ファイル名の文字列**で持っており、リネームで静かに壊れる状態
...
| [`registry/apps.yaml`](registry/apps.yaml) | アプリ／GAS台帳（8件） |
...
1. **ref_style: name の4スキルをID参照に修正** — `track-d-case-mse` / `mse-weekly-kpi-recovery` /
   `mse-shodan-gas` / `alert-overdue-mse`。いま最も壊れやすい
2. **GASソースの Git 取り込み** — clasp で7プロジェクトを `apps/` 配下に同期。現状バックアップ皆無
3. **check-sf-sheet-consistency 系3スキルのデータソース表を台帳参照に一本化** — いま3重複
4. **マイドライブ直下250件の分類** — 重複判定と削除は所有者判断が必要
5. **【井上】個人チェックシート2026 の鮮度確認** — 突合の入力として妥当か
6. **calendar-add-v2 / makeshop-design-guidelines の廃止判断** — 後継と併存している
